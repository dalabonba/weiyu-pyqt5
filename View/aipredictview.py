...
class AimodelView(BaseView):
    ...
    def create_predict(self, parent_layout, current_panel):
        ...
        layout.addLayout(lower_layout)  # 添加下顎佈局
        layout.addLayout(upper_layout)  # 添加上顎佈局
        ...
        save_button.clicked.connect(self.save_ai_file)  # 綁定點擊事件
        layout.addWidget(save_button)  # 添加按鈕到佈局

        panel.setLayout(layout)  # 設置面板佈局
    ...
    def update_view(self):
        ...
        self.model_file.setText(self.model.model_folder)  # 更新模型文件路徑
        self.threeddown_file.setText(self.model.lower_file)  # 更新下顎文件路徑
        self.threedupper_file.setText(self.model.upper_file)  # 更新上顎文件路徑
        self.output_folder.setText(self.model.output_folder)  # 更新輸出文件夾路徑
